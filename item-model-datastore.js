...
var ancestorKey = ds.key(['PatronItem', 'checkedOut']);
...
function getItem(recordID, recordDate, cb) {
    ...
    ds.runQuery(q, function (err, entities, nextQuery) {
        ...
        if (nextQuery.moreResults !== Datastore.NO_MORE_RESULTS || entities.length > 1) {
            console.log('Error: Found ' + entities.length + ' checked out items matching:', recordID, ', ', recordDate);
...
function update (id, data, cb) {
  var key;
  if (id) {
    key = ds.key(['PatronItem', 'checkedOut', kind, parseInt(id, 10)]);
  } else {
    key = ds.key(['PatronItem', 'checkedOut', kind]);
  }
...
function read (id, cb) {
  var key = ds.key(['PatronItem', 'checkedOut', kind, parseInt(id, 10)]);
